...
void main()
{
	...
	n = normalize(normalize(u_model_mat * vertex_normal).xyz);
	v = normalize(camera_position - (u_model_mat * vertex_position).xyz);
	h = normalize(v+l);
	d = length(camera_position - (u_model_mat * vertex_position).xyz);
